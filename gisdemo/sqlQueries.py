...
json_sql_queries = {'allCities':JSON_SQL_Query("SELECT map.name, map.greenspace, ST_AsGeoJSON(map.the_geom) FROM map order by map.greenspace desc limit 10000", ("name", "greenspace", "geom"), [0, 1], ("%s", "%f", ""), 2),
                    'last10Cities':JSON_SQL_Query("SELECT map.name, map.greenspace, ST_AsGeoJSON(map.the_geom), processed.process_end_time FROM map INNER JOIN processed ON map.gid = processed.gid WHERE processed.file_path is not null order by processed.process_end_time desc limit 10;", [0, 1], ("name", "greenspace", "geom", "time"), ("%s", "%f", "" ""), 2)
    }
...
#
    ...
    for resultTuple in results:
        ...
        for item_number in range(0, len(resultTuple)):
            ...
            fmt_string = '"' + json_query.resultsNameTuple[item_number] + '":"' + json_query.fmtTuple[item_number] + '"'
            entries += [fmt_string % resultTuple[itemNumber]]
