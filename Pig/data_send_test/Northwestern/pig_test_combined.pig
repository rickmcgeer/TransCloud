...
A = LOAD 'hdfs://opencirrus-01101.hpl.hp.com:54310/user/hadoop/results/pig_test/total_traffic/part-r-00000' USING PigStorage('\t') AS (title:chararray, total:double);

B = LOAD 'hdfs://131.246.112.35:54310/user/hadoop/results/pig_test/total_traffic/part-r-00000' USING PigStorage('\t') AS (title:chararray, total:double);
...
STORE E INTO 'results/combined/total_traffic';
-- DUMP E;



-- Load the list of protocols / services and the total packet sizes
F = LOAD 'hdfs://opencirrus-01101.hpl.hp.com:54310/user/hadoop/results/pig_test/total_traffic_per_known_protocol/part-r-00000' USING PigStorage('\t') AS (protocol:chararray, protocol_total:double);

G = LOAD 'hdfs://131.246.112.35:54310/user/hadoop/results/pig_test/total_traffic_per_known_protocol/part-r-00000' USING PigStorage('\t') AS (protocol:chararray, protocol_total:double);
...
STORE J INTO 'results/combined/total_traffic_per_known_protocol';
-- DUMP J;
...
STORE K INTO 'results/combined/traffic_percent_per_protocol';
-- DUMP K;



-- Load the total known traffic results
L = LOAD 'hdfs://opencirrus-01101.hpl.hp.com:54310/user/hadoop/results/pig_test/total_known_traffic/part-r-00000' USING PigStorage('\t') AS (title:chararray, total_known_traffic:double);

M = LOAD 'hdfs://131.246.112.35:54310/user/hadoop/results/pig_test/total_known_traffic/part-r-00000' USING PigStorage('\t') AS (title:chararray, total_known_traffic:double);

N = UNION L,M;
O = GROUP N BY title; -- group by port name
P = FOREACH O GENERATE group, SUM(N.total_known_traffic) AS total_known_traffic:double;
STORE P INTO 'results/combined/total_known_traffic';
...
STORE Q INTO 'results/combined/known_traffic_percent_per_protocol';
-- DUMP Q;
