...
A = LOAD 'data/umass1.txt' USING PigStorage('\t') AS (timestamp:double, record_ip_tlen:double, record_ip_id:int, record_ip_ttl:int, record_ip_protocol:int, protocol:chararray, record_ip_src:long, record_ip_dest:long, source_ip:chararray, dest_ip:chararray, source_port:int, dest_port:int, record_tcp_seqno:long, record_tcp_ackno:long, flag1:int, flag2:int, flag3:int, record_tcp_win:int);
